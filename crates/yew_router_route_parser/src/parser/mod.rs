                ...
                RouteParserToken::Separator => {
                    alt((
                        ...
                        .map_err(|_| {nom::Err::Error(ParserError::ExpectedOneOf(vec![
                            ...
                            RouteParserToken::Capture(RefCaptureVariant::NumberedNamed{sections: 0, name: ""}),
                            RouteParserToken::QueryBegin,
                            RouteParserToken::FragmentBegin,
        ...
        ParserState::FirstQuery {prev_token} => {
            match prev_token {
                RouteParserToken::QueryBegin => {
                    query_capture(i)
                        .map_err(|_| {nom::Err::Error(ParserError::ExpectedOneOf(vec![
                            RouteParserToken::QueryCapture{ident: "", capture_or_match: CaptureOrExact::Capture(RefCaptureVariant::Named(""))},
                        ]))})
                }
...
fn numbered_capture_impl(i: &str) -> IResult<&str, RefCaptureVariant> {
    map(
        ...
        |(number, key)| RefCaptureVariant::NumberedNamed{ sections: number.parse().unwrap(), name: key }
    )(i)
}
