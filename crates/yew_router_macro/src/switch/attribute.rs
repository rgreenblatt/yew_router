use syn::{MetaNameValue, Attribute, Meta, Lit};
...
impl AttrToken {
                        ...
                        mnv.path.clone()
                            ...
                            .filter_map(|ident| {
                                match ident.to_string().as_str() {
                                    "to" => Some(AttrToken::To(get_meta_name_value_str(&mnv).expect("Value provided after `to` must be a String"))),
    ...
    pub fn into_shadow_matcher_tokens(self, id: usize) -> Vec<ShadowMatcherToken> {
        match self {
            ...
            AttrToken::Capture(Some(capture_name)) => {
                vec![
            ...
            AttrToken::Rest(Some(capture_name)) => {
                vec![
                    ShadowMatcherToken::Capture(ShadowCapture{capture_variant: ShadowCaptureVariant::ManyNamed(capture_name), allowed_captures: None })
                ]
            }
            AttrToken::Rest(None) => {
                vec![
